...
export default {
    ...
    activate(state) {
        ...
        this.subscriptions.add(atom.commands.add('atom-workspace', {
            'atom-redpen-checker:changeRule': () => this.changeRule()
        }));
    ...
    changeRule() {
        if (!this.selectListView) {
            this.selectListView = new SelectListView({
                items: this.loadRules(),
                elementForItem: (item) => {
                    const li = document.createElement('li')
                    const primary = document.createElement('div')
                    const secondary = document.createElement('div')
                    primary.textContent = item.title
                    primary.setAttribute('class', 'primary-line icon icon-file-text')
                    secondary.textContent = item.path
                    secondary.setAttribute('class', 'secondary-line no-icon')
                    li.setAttribute('class', 'two-lines')
                    li.appendChild(primary)
                    li.appendChild(secondary)
                    return li
                },
                didConfirmSelection: (item) => {
                    this.changedRule = item
                    this.panel.hide();
                },
                didCancelSelection: () => {
                    this.panel.hide();
            ...
            this.selectListView.update({items: this.loadRules()});
        }
        if (this.panel == null) {
    ...
    loadRules() {
        const configPath = `${atom.getConfigDirPath()}/atom-redpen-checker.cson`;
        fs.exists(configPath, (exists) => {
            if (!exists) {
                const store = [];
                try {
                    CSON.writeFileSync(configPath, store);
                } catch (e) {
                    // ignore
                }
            }
        ...
        let data = [{
            title: 'Default',
            path: atom.config.get('atom-redpen-checker.redpenConfigFile')
        }];
        try {
            data = data.concat(CSON.readFileSync(configPath));
        } catch (e) {
            console.log(e);
        }
    ...
    executeRedpen(resultHandler, tmpFile) {
        ...
        const configFile = this.changedRule == null ? atom.config.get('atom-redpen-checker.redpenConfigFile') : this.changedRule.path;
